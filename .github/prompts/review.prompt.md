...
`.claude/commands/review.md` is the single source of truth. Follow it exactly: correctness & convention compliance → test quality & coverage → verify by running `ng build` and `ng test` yourself → architecture & debt trajectory → report in the structured format with verdict APPROVE or REQUEST CHANGES.
