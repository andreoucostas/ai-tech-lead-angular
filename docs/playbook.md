...
Developer types: "add export button to dashboard"
         ...
         Hooks (settings.json)
         tsc --noEmit → catch type errors → self-correct
                    │
...
2. **`FRAMEWORK-CONTEXT.md` is mostly stable.** Maintainer-curated sections rarely change between sessions. The auto-populated "Detected Framework Packages" table changes when package.json changes — usually rare.
3. **Volatile content lives in hooks, not in `CLAUDE.md`.** `SessionStart` injects branch + recent commits + debt heat at session start; `UserPromptSubmit` injects routed-intent rails; `PostToolUse` injects type-check output. All of these append AFTER the CLAUDE.md prefix, preserving cache integrity.
...
- **Build/test pass rate**: hooks should catch type errors early
